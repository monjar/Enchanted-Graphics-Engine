...
Still absent, and tracked per phase in §6: anti-aliasing, point-light and spot shadows, cooked asset packaging, script hot reload, character controllers and physics constraints.
...
| `ege::ShadowMapSystem` | Depth-only sun shadow pass, run once per cascade; the maps are layers of one frame graph transient |
...
- ~~**Cascaded** shadow maps for the sun~~ — **done**, recorded above. **Cube shadow maps for point lights** and spot shadows remain; the frame graph's layered images, added for the cascades, are what they would be built on.
