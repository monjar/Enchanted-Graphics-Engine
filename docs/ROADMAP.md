...
| Item | What it is | Done when |
|---|---|---|
| ~~The backend~~ | **Landed.** miniaudio pinned through CPM, compiled into `MiniaudioEngine.cpp` and nowhere else. The *mixing* is the engine's rather than the library's: miniaudio's high-level engine would do spatialisation and voices for us, and taking it would put "how loud is that" inside a dependency nobody can test against - so what is used is the device callback, and everything above it is `AudioMath` and a mixer small enough to read. The silent backend is a backend, not a stub: it accepts clips, hands out voices and expires them after exactly as long as the clip lasts, so code that waits for a voice gets the same answer either way. miniaudio's *own* null device is refused, because a backend that reports itself audible while discarding everything is worse than one that admits it. | **Done: the suite and the headless run pass on this machine, which has no sound device at all - it takes the silent path and nothing else changes.** |
| ~~Audio assets~~ | **Landed**, with one addition the repository's own rules forced. `.wav`, `.mp3` and `.flac` decode through the backend's decoder, in the same translation unit that knows its name. But a clean checkout ships no binary assets - models are text glTF, materials and scenes are JSON - and a `.wav` would have been the first file in the tree nobody can read in a diff. So there is also `.egesound`: a sound *described* rather than recorded, a few oscillators and an envelope rendered to samples on load, which is what placeholder audio wants anyway. Sounds catalogue, get sidecars, load on the job system (decoding is real work, unlike reading a prefab) and hot-reload; a voice already playing keeps the old samples and finishes on them rather than clicking. | **Done: the browser lists sounds and double-clicking one plays it.** |
