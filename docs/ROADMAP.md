...
Still absent, and tracked per phase in §6: spot shadows, post-process anti-aliasing beyond MSAA (FXAA/TAA), cooked asset packaging, script hot reload, character controllers and physics constraints.
...
- ~~**Cascaded** shadow maps for the sun~~ — **done**, recorded above. ~~**Cube shadow maps for point lights**~~ — **done** too, and built on exactly what was predicted: the frame graph's layered images, taught to be sampled as cubes. Spot shadows remain, and are the smallest of the three.
